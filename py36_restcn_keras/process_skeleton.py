import sys
#sys.path.append('/cm/shared/apps/python/3.6.0/lib/python3.6/site-packages/matplotlib')
...
def vids_with_missing_skeletons():
  f = open('d:/datasets/NTU_RGB+D/NTU_RGBD120_missings.txt','r')
  bad_files = []
  for line in f:
...
def generate_data(argv):
  ...
  skeleton_dir_root = "d:/datasets/NTU_RGB+D/cl60/nturgb+d_skeletons"
  skeleton_files = os.listdir(skeleton_dir_root)
  data_out_dir = 'd:/datasets/NTU_RGB+D/subjects_split_raw/'
  ...
  lmdb_env_x = lmdb.open(lmdb_file_x, map_size=int(1e11))
  lmdb_env_y = lmdb.open(lmdb_file_y, map_size=int(1e11))
